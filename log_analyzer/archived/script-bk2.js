  ...
  Papa.parse(e.target.files[0], {
    ...
    complete: res => {
      ...
      plot.addEventListener('wheel', ev => {
        ...
        const xa = plot._fullLayout.xaxis;
        let [xmin, xmax] = xa.range;
        const span = xmax - xmin;
        const zoom = ev.deltaY < 0 ? 0.85 : 1.15;

        const bb = plot.getBoundingClientRect();
        const pct = (ev.clientX - bb.left) / bb.width;
        const xCenter = xmin + pct * span;

        let newSpan = span * zoom;
        let newMin = xCenter - newSpan * pct;
        let newMax = xCenter + newSpan * (1 - pct);

        newMin = Math.max(xa.minallowed, newMin);
        newMax = Math.min(xa.maxallowed, newMax);

        Plotly.relayout(plot,{'xaxis.range':[newMin,newMax]});
      },{passive:false});
